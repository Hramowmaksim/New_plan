...
# Редактирование груза по двойному клику в таблице
# ------------------------------------------------------------------
    
    # ------------------------------------------------------------------
# РЕДАКТИРОВАНИЕ ЯЧЕЙКИ ПРЯМО В ТАБЛИЦЕ
# ------------------------------------------------------------------
# ------------------------------------------------------------------
# Inline-редактирование ячеек Treeview
# ------------------------------------------------------------------
    def on_cell_double_click(self, event):
        ...
        # ID строки и ключ колонки для сохранения
    ...
    def save_cell_edit(self, event=None):
        ...
        try:
            ...
            # Если изменяли размеры или вес — удаляем экземпляры
            if col in ("l", "w", "h", "wt", "qty"):
                ...
                cargo['instances'].clear()
                cargo['placed'] = 0

            # Обновить значение
            cargo[col if col != "l" else "length"] = new_val
            cargo[col if col != "w" else "width"] = new_val
            cargo[col if col != "h" else "height"] = new_val
            cargo[col if col != "wt" else "weight"] = new_val
            cargo[col if col != "qty" else "qty"] = new_val
    ...
    # def edit_cargo_from_table(self, event):
    #     region = self.tree.identify("region", event.x, event.y)
    #     if region != "cell":
    #         return
    ...
    #     win.title("Редактировать груз")
    #     win.grab_set()  # модально

    #     entries = {}
    #     labels = ("Название:", "Количество:", "Длина (мм):", "Ширина (мм):",
    #             "Высота (мм):", "Вес (кг):")
    #     keys   = ('name', 'qty', 'length', 'width', 'height', 'weight')
    #     values = [cargo[k] for k in keys]
    ...
    #     def save_changes():
    #         try:
    #             new_name = validate_name(entries['name'].get())
    #             new_qty  = int(entries['qty'].get())
    #             new_l    = int(entries['length'].get())
    #             new_w    = int(entries['width'].get())
    #             new_h    = int(entries['height'].get())
    #             new_wt   = int(entries['weight'].get())

    #             if new_qty < 0 or min(new_l, new_w, new_h, new_wt) <= 0:
    #                 raise ValueError

    #             # Удалить старые экземпляры с холстов
    #             for inst in cargo.get('instances', []):
    #                 for canv in (self.top_canvas, self.side_canvas, self.front_canvas):
    #                     canv.delete(inst['top_id'])
    #                     canv.delete(inst['side_id'])
    ...
    #             win.destroy()
    #         except ValueError:
    #             messagebox.showerror("Ошибка", "Проверьте ввод", parent=win)

    #     tk.Button(win, text="Сохранить", command=save_changes).grid(
    #         row=len(labels), column=0, columnspan=2, pady=10)
    #     tk.Button(win, text="Отмена", command=win.destroy).grid(
...
# не работает перерисовка после изменения размеров
