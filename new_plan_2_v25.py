...
# добавление функций вставки 

        self.clipboard = []          # [(cargo_id, dx, dy, dz, l, w, h, weight, name), ...]
    ...
    # ------------------------------------------------------------------
    # Копирование выделенных коробок (Ctrl+C)
    # ------------------------------------------------------------------
    def copy_selected(self, event=None):
        if not self.selected:
            return
        self.clipboard.clear()
        # якорь = левый-нижний угол группы
    ...
    def paste_clipboard(self, event=None):
        ...
        # переводим координаты курсора в мм
        if mode == 'front':
            x_mm = (cx - 10) / (SCALE * 2)
...
# добавлена возможность копировать вставить 
# надо убрать окошко с уведомлением и исправить нюансы отрисовки (при перетаскивании отрисовывается в неправильном порядке)
# следующий майлстоун это добавление расчета центра тяжести 
# так же надо подрезать картинки подложки и сделать их более читаебельными 
