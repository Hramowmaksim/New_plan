...
# Зазор между коробками = 10 мм.
    ...
    def do_drag(self, evt, mode):
        ...
        # сдвиг в пикселях с момента нажатия
        ...
        # единый сдвиг для всех коробок группы
        for cid, idx in self.drag_keys:
            c = next(c for c in self.cargos if c['id'] == cid)
            inst = c['instances'][idx]

            # начальные координаты именно этой коробки
            base_x = inst['x']
            base_y = inst['y']
            base_z = inst['z']

            if mode == 'top':
                new_x = max(0, min(self.L - inst['length'], base_x + dx_mm))
                new_y = max(0, min(self.W - inst['width'],  base_y - dy_mm))
                new_z = self.drop_z(new_x, new_y, inst['length'], inst['width'], exclude_instance=inst)
            elif mode == 'side':
                new_x = max(0, min(self.L - inst['length'], base_x + dx_mm))
                new_z = max(0, min(self.H - inst['height'], base_z - dy_mm))
                new_y = base_y
            elif mode == 'front':
                new_y = max(0, min(self.W - inst['width'],  base_y + dx_mm))
                new_z = max(0, min(self.H - inst['height'], base_z - dy_mm))
                new_x = base_x
        ...
        for cid, idx in self.drag_keys:
            ...
            if mode == 'top':
                ...
                new_x = max(0, min(self.L - inst['length'], inst['x'] + dx_mm))
                new_z = max(0, min(self.H - inst['height'], inst['z'] - dy_mm))
                new_y = inst['y']
            elif mode == 'front':
