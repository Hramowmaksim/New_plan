...
class ContainerApp:
    def __init__(self, root):
        ...
        self.group_anchor = (0, 0, 0)   # якорь группы (x, y, z)
...
# ------------------------------------------------------------------
    ...
    def delete_selected_rects(self, event=None):
        ...
        # сортируем по убыванию индекса, чтобы не нарушать порядок
...
# Зазор между коробками = 10 мм.
    ...
    def start_drag(self, evt, mode):
        ...
        # формируем список перетаскиваемых
        if self.selected:
        ...
        # запоминаем точку, где пользователь нажал ЛКМ (в пикселях)
    ...
    def do_drag(self, evt, mode):
        ...
        # фильтруем живые пары
        self.drag_keys = [(cid, idx) for cid, idx in self.drag_keys
                        if any(c['id'] == cid and idx < len(c.get('instances', []))
                                for c in self.cargos)]
        ...
        scale = SCALE * (2 if mode == 'front' else 1)
        dx_mm = dx_px / scale
        dy_mm = dy_px / scale

        # 1. bounding-box группы
        ...
        for cid, idx in self.drag_keys:
            ...
            max_x = max(max_x, inst['x'] + inst['length'])
            min_y = min(min_y, inst['y'])
            max_y = max(max_y, inst['y'] + inst['width'])
            min_z = min(min_z, inst['z'])
        ...
        # 2. новое положение левого-нижнего угла bounding-boxа
        new_bx = new_by = new_bz = 0  # заглушка-инициализация

        if mode == 'top':
            new_bx = min_x + dx_mm
            new_by = min_y - dy_mm
            new_bz = min_z  # при top Z не меняется
        elif mode == 'side':
            new_bx = min_x + dx_mm
            new_by = min_y
            new_bz = min_z - dy_mm
        ...
        # 3. обрезаем до границ
        new_bx = max(0, min(self.L - box_l, new_bx))
        new_by = max(0, min(self.W - box_w, new_by))
        new_bz = max(0, min(self.H - box_h, new_bz))

        # 4. опускаем на опору
        new_bz = self.drop_z(new_bx, new_by, box_l, box_w,
                            exclude_instances=valid_insts)
        new_bz = max(0, new_bz)

        # 5. коллизия с остальными
        if self.collision_3d(new_bx, new_by, new_bz, box_l, box_w, box_h,
                            exclude_instances=valid_insts):
            return

        # 6. применяем сдвиг
        for inst in valid_insts:
            offset_x = inst['x'] - min_x
            offset_y = inst['y'] - min_y
            offset_z = inst['z'] - min_z
            self.move_instance_to(inst,
                                new_bx + offset_x,
                                new_by + offset_y,
                                new_bz + offset_z)
    ...
    def drop_z(self, x, y, l, w, exclude_instance=None):
        """
        Возвращает минимальную высоту z, на которую можно опустить коробку
        без пересечения с другими (но не выше текущей).
        """
        # нижняя граница контейнера
        min_z = 0

        # проходим по всем уже размещённым
        for c in self.cargos:
            for inst in c.get('instances', []):
                if inst is exclude_instance:
                    continue
                ...
                if (x < inst['x'] + inst['length'] and x + l > inst['x'] and
                    ...
                    # коробка лежит выше препятствия
                    min_z = max(min_z, inst['z'] + inst['height'])
        return min_z
