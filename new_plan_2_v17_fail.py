...
class ContainerApp:
    def __init__(self, root):
        ...
        self.zoom_mode = False          # режим zoom-рамки
        self.current_scale = 1.0        # текущий коэффициент
        self.zoom_rect_id = None        # ID рамки на canvas
    ...
    def build_gui(self):
        ...
        # ---------- кнопка Zoom ----------
        zoom_row = tk.Frame(left)
        zoom_row.pack(pady=5)
        self.zoom_btn = tk.Button(
            zoom_row, text="Zoom", width=12, command=self.toggle_zoom_mode)
        self.zoom_btn.pack(side=tk.LEFT, padx=3)
        ...
        # ---------- подписки ----------
        self.root.bind_all("<KeyPress-Escape>", self.exit_zoom_mode)
        for canv in (self.top_canvas, self.side_canvas, self.front_canvas):
            canv.bind("<Button-1>", self.zoom_start_rect, add="+")
            canv.bind("<B1-Motion>", self.zoom_drag_rect, add="+")
            canv.bind("<ButtonRelease-1>", self.zoom_apply, add="+")
            canv.bind("<Button-3>", self.zoom_apply, add="+")
        ...
        for canv, tag in ((self.top_canvas,  'top'),
            ...
            canv.bind("<Button-1>", lambda e, t=tag: self.start_drag(e, t), add="+")
            canv.bind("<B1-Motion>", lambda e, t=tag: self.do_drag(e, t), add="+")
            canv.bind("<ButtonRelease-1>", lambda e, t=tag: self.end_drag(e, t), add="+")

            canv.bind("<Button-3>", lambda e, t=tag: self.select_start_rect(e, t), add="+")
            canv.bind("<B3-Motion>", lambda e, t=tag: self.select_drag_rect(e, t), add="+")
            canv.bind("<ButtonRelease-3>", lambda e, t=tag: self.select_end_rect(e, t), add="+")
...
# ------------------------------------------------------------------
    ...
    def select_end_rect(self, evt, mode):
        ...
        # собираем уникальные ключи (cargo_id, idx)
        new_selection = set()
        for c in self.cargos:
            for idx, inst in enumerate(c['instances']):
                ix0 = 10 + inst['x'] * SCALE
                iy0 = 10 + (self.W - inst['width'] - inst['y']) * SCALE
                ix1 = 10 + (inst['x'] + inst['length']) * SCALE
                iy1 = 10 + (self.W - inst['y']) * SCALE
    ...
    def clear_selection(self, evt=None):
        for inst in list(self.selected):
    ...
    def toggle_zoom_mode(self):
        ...
        self.zoom_btn.config(relief='sunken' if self.zoom_mode else 'raised')
        if not self.zoom_mode:
            self.clear_zoom_rect()

    def exit_zoom_mode(self, evt=None):
        if self.zoom_mode:
            self.toggle_zoom_mode()
    # ------------------------------------------------------------------
    # Zoom-рамка
    # ------------------------------------------------------------------
    def clear_zoom_rect(self):
        for c in (self.top_canvas, self.side_canvas, self.front_canvas):
            if self.zoom_rect_id:
                c.delete(self.zoom_rect_id)
        self.zoom_rect_id = None

    def zoom_start_rect(self, evt):
        if not self.zoom_mode:
            return
        self.clear_zoom_rect()
        self.zoom_start = (evt.x, evt.y)
        self.zoom_rect_id = evt.widget.create_rectangle(
            evt.x, evt.y, evt.x, evt.y, outline="red", width=2, dash=(4, 2))

    def zoom_drag_rect(self, evt):
        if not self.zoom_mode or self.zoom_rect_id is None:
            return
        x0, y0 = self.zoom_start
        evt.widget.coords(self.zoom_rect_id, x0, y0, evt.x, evt.y)

    def zoom_apply(self, evt):
        if not self.zoom_mode:
            return
        if evt.type.name == 'ButtonRelease' and self.zoom_rect_id is None:
            return  # не было рамки

        if evt.type.name == 'ButtonRelease':
            # Завершили рамку
            x0, y0 = self.zoom_start
            x1, y1 = evt.x, evt.y
            x0, x1 = sorted((x0, x1))
            ...
            cw = evt.widget.winfo_width() - 20
            ch = evt.widget.winfo_height() - 20
            if cw <= 0 or ch <= 0:
                return
            sx = cw / max(x1 - x0, 1)
            sy = ch / max(y1 - y0, 1)
            factor = min(sx, sy)
            if evt.num == 1:          # ЛКМ
                self.current_scale = max(0.5, min(5.0, self.current_scale * factor))
            elif evt.num == 3:        # ПКМ
                self.current_scale = max(0.5, self.current_scale / factor)
    ...
    def redraw_all(self):
        ...
        # размеры canvas
        self.top_canvas.config(scrollregion=(0, 0, tw + 20, th + 20))
        self.side_canvas.config(scrollregion=(0, 0, sw + 20, sh + 20))
        self.front_canvas.config(scrollregion=(0, 0, fw + 20, fh + 20))
        ...
        # подложки
        self.draw_containers_with_scale(s)

        # грузы
        for cargo in self.cargos:
            for inst in cargo.get('instances', []):
                self.draw_instance(inst, s)

    def draw_containers_with_scale(self, s):
        from PIL import Image, ImageTk
        import os
        base_dir = os.path.dirname(__file__)
        bg_dir = os.path.join(base_dir, "resources")
        for key, canv, w, h in (('top', self.top_canvas, int(self.L*s), int(self.W*s)),
                                ('side', self.side_canvas, int(self.L*s), int(self.H*s)),
                                ('front', self.front_canvas, int(self.W*s*2), int(self.H*s*2))):
            img = Image.open(os.path.join(bg_dir, f"{key}.png")).convert("RGBA")
            self.bg_img[key] = ImageTk.PhotoImage(img.resize((w, h), Image.LANCZOS))
            canv.create_image(10, 10, anchor='nw', image=self.bg_img[key])
            canv.create_rectangle(10, 10, 10 + w, 10 + h, width=2, outline="black")

    def draw_instance(self, inst, s):
        x, y, z = inst['x'], inst['y'], inst['z']
        l, w, h = inst['length'], inst['width'], inst['height']
        color = "#00B050" if any(c['id'] == cid and idx == i
                                 for cid, i in self.selected) else "#0078D4"
        inst['top_id'] = self.top_canvas.create_rectangle(
            10 + x * s, 10 + (self.W - w - y) * s,
            10 + (x + l) * s, 10 + (self.W - y) * s,
            fill=color, outline="black", tags=("cargo",))
        inst['side_id'] = self.side_canvas.create_rectangle(
            10 + x * s, 10 + (self.H - h - z) * s,
            10 + (x + l) * s, 10 + (self.H - z) * s,
            fill=color, outline="black", tags=("cargo",))
        inst['front_id'] = self.front_canvas.create_rectangle(
            10 + y * s * 2, 10 + (self.H - h - z) * s * 2,
            10 + (y + w) * s * 2, 10 + (self.H - z) * s * 2,
            fill=color, outline="black", tags=("cargo",))
    ...
    def zoom_apply(self, evt):
        ...
        if evt.type.name == 'ButtonRelease':
            ...
            sy = ch / max(y1 - y0, 1)
            factor = min(sx, sy)
            if evt.num == 1:          # ЛКМ — zoom in
                self.current_scale = max(0.5, min(5.0, self.current_scale * factor))
            elif evt.num == 3:        # ПКМ — zoom out
                self.current_scale = max(0.5, self.current_scale / factor)
    ...
    def redraw_all(self):
        ...
        for c, w, h in ((self.top_canvas, tw, th),
                        (self.side_canvas, sw, sh),
                        (self.front_canvas, fw, fh)):
            c.config(scrollregion=(0, 0, w + 20, h + 20))
            c.delete("all")
