...
class ContainerApp:
    ...
    def build_gui(self):
        ...
        # ПРАВАЯ – рамка
        self.top_canvas.bind("<Button-3>", self.select_start_rect)   # правая
        self.top_canvas.bind("<B3-Motion>", self.select_drag_rect)   # правая
        self.top_canvas.bind("<ButtonRelease-3>", self.select_end_rect)  # правая
    ...
    def start_drag(self, evt):
        ...
        # найти ключ (cargo_id, idx) по top_id
        for c in self.cargos:
            for idx, inst in enumerate(c['instances']):
                if inst['top_id'] == clicked_id:
                    key = (c['id'], idx)
                    if key in self.selected:
                        self.drag_keys = list(self.selected)   # вся группа
                    else:
                        self.drag_keys = [key]               # один
    ...
    def do_drag(self, evt):
        ...
        for cid, idx in self.drag_keys:
            ...
            new_x = max(0, min(self.L - l, inst['x'] + dx))
            new_y = max(0, min(self.W - w, inst['y'] - dy))
            new_z = self.drop_z(new_x, new_y, l, w,
                                exclude_instance=inst)

            if self.collision_3d(new_x, new_y, new_z, l, w, h,
                                 exclude_instance=inst):
                return   # отменяем движение всей группы
    ...
    def end_drag(self, evt):
        ...
        l = self.drag_instances['length']
        w = self.drag_instances['width']

        new_x = max(0, min(self.L - l, self.drag_instances['x'] + dx))
        new_y = max(0, min(self.W - w, self.drag_instances['y'] - dy))

        # «падаем» на самый верх в этой точке
        new_z = self.drop_z(new_x, new_y, l, w, exclude_instance=self.drag_instances)

        # проверка коллизии (исключаем самого себя)
        if self.collision_3d(new_x, new_y, new_z, l, w,
                             self.drag_instances['height'],
                             exclude_instance=self.drag_instances):
            return   # на всякий случай, если вдруг пересечение

        self.move_instance_to(self.drag_instances, new_x, new_y, new_z)
        self.drag_instances = None
    ...
    def select_start_rect(self, evt):
        # если не shift – сбросить предыдущее выделение
        if not (evt.state & 0x0001):
            self.clear_selection()
        self.select_start = (evt.x, evt.y)
        if self.select_rect:
            self.top_canvas.delete(self.select_rect)
        self.select_rect = None

    def select_drag_rect(self, evt):
        if self.select_rect:
            self.top_canvas.delete(self.select_rect)
        x0, y0 = self.select_start
        self.select_rect = self.top_canvas.create_rectangle(
            x0, y0, evt.x, evt.y, outline="green", width=2, dash=(4, 2))

    def select_end_rect(self, evt):
        if self.select_rect:
            self.top_canvas.delete(self.select_rect)
            self.select_rect = None
        x0, y0 = self.select_start
    ...
    def recolor_selected(self):
        for inst in self.selected:
            self.top_canvas.itemconfig(inst['top_id'], fill="#00B050")
            self.side_canvas.itemconfig(inst['side_id'], fill="#00B050")
            self.front_canvas.itemconfig(inst['front_id'], fill="#00B050")
